...
def get_n_save_accuracy(X_test, y_test, test_name, dataset_name):
    model = tf.keras.models.load_model(OUTPUT_PATH + "models/" + test_name + ".h5")
...
def get_n_save_confusion_matrix(
    X_test, y_test, y_pred, test_name, dataset_name, should_delete_rows=False
):
    ...
    if dataset_name == "German":
        disp.plot(cmap=plt.cm.Blues, ax=ax, values_format=".1g")
    elif dataset_name == "Chinese":
        disp.plot(cmap=plt.cm.Greens, ax=ax, values_format=".1g")
    elif dataset_name == "Belgian":
        disp.plot(cmap=plt.cm.RdPu, ax=ax, values_format=".1g")
    plt.xticks(rotation=90)
    plt.title(dataset_name + "_" + test_name)
    plt.savefig(
        OUTPUT_PATH + "confusion_matrix/" + test_name + "_" + dataset_name + ".png"
    )
...
def readDsBelgianByClass(belgianClass):
    ...
    y_test = pd.read_csv(
        "datasets/Belgian_dataset/BelgiumTSC_Training/Training/"
        + name
        + "/newGT-"
        + name
        + ".csv"
...
xnor_model_names = [
    "_QConv_16_3_MP_2_QConv_32_2_MP_2_BN_Dense_43_ep_30",
    "_QConv_32_3_MP_2_QConv_64_2_MP_2_BN_Dense_43_ep_30",
    "_QConv_64_3_MP_2_QConv_128_2_MP_2_BN_Dense_43_ep_30",
]
...
chinese_x_test, chinese_y_test = get_dataset(
    "datasets/Chinese_dataset/annotations_v5.csv",
    "category",
    "file_name",
    "datasets/Chinese_dataset/images/",
)
belgium_x_test, belgium_y_test = getLabelsAndImagesBelgian()

for xnor_model in xnor_model_types:
    for model_name in xnor_model_names:
